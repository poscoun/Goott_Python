...
'''
# calendar 모듈 : 파이썬에서 달력을 표시해주는 모듈
calendar.calendar(해당년도) : 해당년도의 전체 달력을 출력
calendar.prcal(해당년도) : 해당년도의 전체 달력을 출력
calendar.prmonth(년도, 월) : 해당년도의 월만 출력
calendar.weekday(년도, 월, 일) : 입력받은 날짜의 요일을 반환 (숫자로 반환)
...
# webbrowser : 자신의 시스템에 사용하는 기본 웹브라우저가 자동으로 실행되게 하는 모듈
